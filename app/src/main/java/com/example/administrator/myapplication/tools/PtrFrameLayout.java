...
public class PtrFrameLayout extends ViewGroup {
    ...
    private OnPtrScrollListener mOnPtrScrollListener;
     ...
     * 是否点击在懒View上
     */
    private boolean touchInLazyView = false;
    
    /**
     * 点击在懒View上，进行垂直滚动,0等待滑动,1垂直滑动,2水平滑动
     */
    private int touchInLazyViewToVerticalScroll = 0;

	public boolean isTouchInLazyView() {
		return touchInLazyView;
	}

	public void setTouchInLazyView(boolean touchInLazyView) {
		this.touchInLazyView = touchInLazyView;
	}

	public int isTouchInLazyViewToVerticalScroll() {
		return touchInLazyViewToVerticalScroll;
	}

	public void setTouchInLazyViewToVerticalScroll(
			int touchInLazyViewToVerticalScroll) {
		this.touchInLazyViewToVerticalScroll = touchInLazyViewToVerticalScroll;
	}

	public void setOnPtrScrollListener(OnPtrScrollListener listener){
    	mOnPtrScrollListener = listener;
    }
    ...
    public boolean dispatchTouchEvent(MotionEvent e) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_CANCEL:
                ...
                touchInLazyView = false;
                touchInLazyViewToVerticalScroll = 0;
            ...
            case MotionEvent.ACTION_DOWN:
                ...
                // fix #93, #102
                boolean isTouchLazyView = false;
                for (View view : useLazyPagingTouchSlopView) {
                	isTouchLazyView = inRangeOfView(view,e);
                	if(isTouchLazyView)
                		break;
				}
                if(isTouchLazyView){
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (isTouchInLazyView()) {
                	if(touchInLazyViewToVerticalScroll == 0){
                		if(Math.abs(offsetX) > Math.abs(offsetY)){
                			touchInLazyViewToVerticalScroll = 2;
                		}else if(Math.abs(offsetX) < Math.abs(offsetY)){
                			touchInLazyViewToVerticalScroll = 1;
                		}else if(offsetX == 0 && offsetY == 0){
                			touchInLazyViewToVerticalScroll = 0;
                		}
                	}
                	if(touchInLazyViewToVerticalScroll == 2){
                		return dispatchTouchEventSupper(e);
                	}
                }else{
                	if ((Math.abs(offsetX) > mPagingTouchSlop)) {
    ...
    private void movePos(float deltaY) {
        ...
        if(mOnPtrScrollListener != null){
        	mOnPtrScrollListener.onScrollChangeListener(to);
        }
    ...
    List<View> useLazyPagingTouchSlopView = new ArrayList<View>();
    
    public void addUseLazyPapingTouchSlopView(View view){
    	useLazyPagingTouchSlopView.add(view);
    }
    ...
    public interface OnPtrScrollListener{
    	public void onScrollChangeListener(int dy);
    }
}
